layui.use(['upload','form','element','layer','flow'], function(){
		...
		var uploadInst = upload.render({
            ...
            ,accept:'file'
            ,acceptMime:'image/*'
            ,exts: 'jpg|jpeg|png|gif|ico|bmp|svg|webp'
            ,size:102400
...
function handleres(res,index){
    layui.use('layer', function(){
        ...
        if(res.code != 'success'){
            ...
            layer.closeAll('loading'); 
            $("#img-thumb a").attr('href',res.data.cdn_url);
            $("#img-thumb img").attr('src',res.data.cdn_url);
...
document.addEventListener('paste', function (event) {
    ...
    if ( event.clipboardData || event.originalEvent ) {
        ...
        if ( clipboardData.items ) {
            ...
            if(images.length > 0) {
                layer.confirm('是否上传剪贴板文件？', function(index){
                    layer.load();
